...
const actions = {
  ...
  login({ dispatch, commit ,state}, userInfo) {
    const { username, password,letOut } = userInfo
    console.log("login---------userInfo",userInfo)
    console.log("login---------token",state.token)

    return new Promise((resolve, reject) => {
      login({ username: username.trim(), password: password,letOut:letOut }).then(response => {
        const { data } = response
  ...
  getInfo({ commit, state }) {
    return new Promise((resolve, reject) => {
      getInfo().then(response => {
        ...
        commit('SET_ACCOUNT',profile.account)
        commit('SET_USERID',profile.id)
        resolve(data)
      }).catch(error => {
        reject(error)
      })
